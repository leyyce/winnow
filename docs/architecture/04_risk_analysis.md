...
| Health endpoint at `/api/v1/health` (canonical) + `/health` (alias) | Moves the versioned health endpoint inside the v1 router for future response schema versioning, while preserving the bare `/health` path as a permanent alias for Docker `HEALTHCHECK`, load balancers, and Kubernetes probes. Zero breaking changes. | Health at root `/health` only (rejected: no versioning path); health at `/api/v1/health` only (rejected: breaks all existing infrastructure healthcheck configs). |
| Dev venv at `/opt/venv` with named Docker volume | Moving the container venv outside the `/app` source tree eliminates the anonymous volume footgun where `docker compose down -v` was required after every dependency change. A named `dev_venv` volume survives `docker compose down` and is refreshed by `docker compose up --build`. | Anonymous volume at `/app/.venv` (rejected: stale env after dep changes, nuclear `down -v` required, destroys unrelated Caddy volumes). |
| `AwareDatetime` for all timestamp fields in schemas | Pydantic V2's `AwareDatetime` rejects naive datetimes (no timezone offset) at validation time, preventing silent downstream errors in timezone-aware comparisons and ISO-8601 serialisation. | Plain `datetime` (rejected: silently accepts naive datetimes, produces incorrect timestamps in UTC comparisons). |
